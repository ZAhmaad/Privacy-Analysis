...
import { Label, expandLabel } from "./label";
...
function _createExpandUrlInfo(
  baseUrl: string
): (compact: _CompactUrlInfo) => UrlInfo {
  return (compact) => ({ url: new URL(compact.url, baseUrl) });
}
...
function _createExpandStorageSourceInfo(
  baseUrl: string
): (compact: _CompactStorageSourceInfo) => StorageSourceInfo {
  return (compact) => ({
    key: compact.key,
    value: compact.value,
    ownership: compact.ownership ? new URL(compact.ownership, baseUrl) : null,
  });
}
...
function expandAnalysisLabel(compact: CompactLabel, baseUrl: string): Label {
  switch (compact.type) {
    ...
    case SOURCE_SESSIONSTORAGE_GETITEM:
      return expandLabel(
        ...
        _createExpandStorageSourceInfo(baseUrl)
      );
    ...
    case SINK_HTMLELEMENT_SRC:
      return expandLabel(compact, _validateCompactValueInfo, (compact) =>
        _createExpandUrlInfo(baseUrl)({ url: compact.value })
      );
